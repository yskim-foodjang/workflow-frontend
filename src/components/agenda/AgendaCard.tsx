...
export default function AgendaCard({ agenda }: AgendaCardProps) {
  ...
  return (
    <Link to={`/agendas/${agenda.id}`} className="block">
      <Card padding="sm" className="flex gap-3 hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className={clsx('font-medium truncate', agenda.isCompleted ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-900 dark:text-white')}>
              {agenda.title}
            </h3>
